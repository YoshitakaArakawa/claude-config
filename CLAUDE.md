...
**同期対象外**: `settings.json` は sync 対象から除外している（アプリが machine-local なキーを書き込むため drift が常態化する）。`skills/tableau-research/references/env-facts.md` も除外している（実環境の値は `~/.claude/` 側で記入し、repo にはテンプレートだけを置く。Public 化されうる repo に実環境情報を commit しないため）。repo の `home/.claude/settings.json` は意図した hooks 設定の記録であり、`~/.claude/settings.json` への反映は手動で行う。逆向きに取り込みたいときだけ `pull-sync.ps1 -Paths settings.json` で明示的に pull する。
