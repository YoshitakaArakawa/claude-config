...
- **絶対パス・個人マシンパス**: `C:\Users\<name>\...` / `/Users/<name>/...` / `/home/<name>/...` / `file:///C:/...` 等の OS 絶対パスを Skill 本体・reference・スクリプト・コメントのいずれにも書かない。リポ内参照は相対パス、同梱スクリプトは `${CLAUDE_SKILL_DIR}`（「文字列置換とエスケープ」参照）、外部参照は抽象的な参照名で記述する。理由: 個人マシン情報・ユーザー名の漏洩、Skill 配布時にリンク先が他環境に存在しなくなる不可搬性。
...
| フィールド | 説明 |
|---|---|
| `when_to_use` | description への追記。skill 一覧では description と合算 1,536 文字で切り詰め |
| `argument-hint` | autocomplete に表示される引数ヒント。例：`[issue-number]` |
| `arguments` | `$name` 置換用の名前付き位置引数 |
| `disable-model-invocation` | `true` で Claude による自動起動を禁止。手動 `/name` のみ。subagent への preload や scheduled task からの起動も無効になる |
| `user-invocable` | `false` で `/` メニューから隠す（Claude のみ起動可） |
| `allowed-tools` | Skill 有効時に承認なしで使えるツール。利用可能ツールを制限するものではない |
| `disallowed-tools` | Skill 有効中、利用可能プールから除去するツール。次のユーザーメッセージで解除される |
| `model` | この Skill 有効時に使うモデル。そのターン限りで settings には保存されない。`/model` と同じ値か `inherit` |
...
| `shell` | 動的コンテキスト注入の実行シェル。`bash`（既定）または `powershell`（`CLAUDE_CODE_USE_POWERSHELL_TOOL=1` が必要） |

### 起動制御の使い分け

- 副作用があり実行タイミングを人が握るべきワークフロー（deploy・commit・メッセージ送信など）→ `disable-model-invocation: true`
- コマンドとして起動する意味がない背景知識 → `user-invocable: false`
...
SKILL.md 本文は invoke 時にレンダリングされ、以下の変数が置換される。references/ 配下はレンダリング対象外（Read で読まれるだけ）なので置換されない。

| 変数 | 展開結果 |
|---|---|
| `$ARGUMENTS` | 引数全体。本文に含まれない場合、引数は本文末尾に `ARGUMENTS: <値>` として追記される |
| `$ARGUMENTS[N]` / `$N` | 0 始まりの位置引数（`$0` が最初）。shell 風クオートで複数語を 1 引数にできる |
...
リテラルの `$ARGUMENTS`・`$<数字>`・宣言済み `$name` を本文に書くには、直前に backslash を置く（`\$ARGUMENTS`）。

**メタスキルの罠**：Skill の書き方を説明する Skill が SKILL.md 本文で置換構文を例示すると、invoke 時に例文が実引数で置換されて壊れる。例示は必ずエスケープするか、reference 側（置換されない）に置く。

## 動的コンテキスト注入

SKILL.md 本文の !`command`（行頭または空白直後でのみ認識）は、Claude が本文を受け取る**前に**シェルで実行され、その出力に置き換わる。live データ（diff・PR 情報・環境情報）を焼き込んでから読ませる用途に使う。

- 複数行コマンドは `` ```! `` で開くフェンスブロックで書く
- 置換は元ファイルに対する 1 パスのみ。コマンド出力内のプレースホルダは再展開されない
- 実行シェルは `shell` フィールドで指定する（フロントマターのフィールド表を参照）
- references/ 配下では実行されない（レンダリング対象は SKILL.md 本文のみ）
- 置換構文と同じメタスキルの罠がある：SKILL.md 本文にこの構文を例示すると実行されてしまう。例示は reference 側に置く
...
- invoke されると本文はレンダリング済みの 1 メッセージとして会話に入り、セッション終了まで残り続ける。**本文の毎行が恒常的なトークンコスト**
- 内容が同一なら再 invoke で本文は再掲されない（「ロード済み」の注記のみ追加される）
- auto-compaction 時、各 Skill は先頭 5,000 トークンまで・全 Skill 合計 25,000 トークンの予算内で再添付される。予算は最近 invoke した Skill から充当され、古いものは落ちることがある

帰結：重要な規範ほど本文の先頭に置く。タスク全体に効くべき指示は「一回きりの手順」ではなく常時有効な規範として書く。
...
- 出力形式が重要なら、テンプレートを本文または同梱ファイルで示す。厳密さは要件に合わせて明示する：「必ずこの構造を使う」（厳格）か「妥当なデフォルト、状況に応じて調整可」（柔軟）か
- スタイルや粒度が重要なら、入出力ペアの例を 2〜3 個示す。説明の言葉を重ねるより例のほうが確実に伝わる
...
- **トリガー試験**：起動すべきプロンプトと起動すべきでないプロンプトを数個ずつ用意し、**fresh session** で試す。作成中セッションに残った文脈はギャップを隠すため、必ず新しいセッションで確認する
- **baseline 比較**：同じプロンプトを Skill 有効 / 無効で実行して比較する。無効化は SKILL.md を編集せず settings の `skillOverrides`（`"off"`）で行える
- **eval runner**：体系的に測るなら公式 skill-creator plugin を使う。テストケース作成（evals.json）→ 隔離実行 → 採点 → with/without ベンチマーク → description tuning までを自動化する。creating-skills は規範の集約を担い、計測は skill-creator に任せる
