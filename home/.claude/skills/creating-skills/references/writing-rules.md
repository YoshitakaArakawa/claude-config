...
Skill は Claude だけが読むものではない。レビューする著者・配布先の利用者も読む。認知負荷を下げる次の設計は、いずれも機械可読性と両立する：

- **規範が先、背景が後**：各節は適用すべき規範から書き始め、理由・背景・例外は後ろに置く。読者は規範だけ拾って読み進められ、compaction 時に先頭が残る効果（「ロード後のライフサイクル」の帰結）とも一致する
- **一文一義**：1文に載せる規範は1つ。読点で複数の規範をつないだ文は分割する
- **列挙の型を使い分ける**：並列3項目以上は文中に詰めず箇条書きにする。属性の比較は表、順序のある手順は番号リストで書く
- **見出しは h3 まで、1節1トピック**：h4 以深が必要になったら、節の分割か reference への切り出しを検討する
- **抽象的な規範には良/悪の対比例を付ける**：説明の言葉を重ねるより例が確実に伝わる（「出力テンプレートと入出力例」も参照）
...
- **トリガー試験**：起動すべきプロンプトと起動すべきでないプロンプトを数個ずつ Skill 内の `evals/trigger-prompts.md` に保存し、**fresh session** で試す。作成中セッションに残った文脈はギャップを隠すため、必ず新しいセッションで確認する。description や本文の修正後は同じセットで回帰確認し、運用中に観測された不発・誤発火のプロンプトを追記して資産化する
